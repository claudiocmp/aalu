class TERRAIN { //<>// //<>// //<>// //<>// //<>//
  ...
  void initialise_terrain(String csv_data_path, String x_header, String y_header, String z_header)
  {
    ...
    {
      throw new RuntimeException("Your CSV does not contain consistent data for the terrain");
    }
    ...
    /*
    TO DO:
     built the vertices[][] array after the csv has been read, in order to introduce
     a sort(by x, by y) to have a horizontal scanning of the terrain automatically 
     */
    ...
    //get vertex tree to run closest pt
    vertex_tree = mesh.getVertexTree();
  }
  ...
  //////////////
  //////////////
  //colour gradient from intervals
  color[] colourByInterval(int intervals, color from, color to ) {
    color[] colours = new color[intervals];
    for (int i=0; i < intervals; i++) {
      colours[i] = lerpColor(from, to, (float)i/(intervals-1));
    }
    return colours;
  }
  //colour gradient from int[]
  color[] colourByValues(int[] topo_layer, color from, color to) {
    //take min-max to map colour on
    int delta = max(topo_layer) - min(topo_layer);
    color[] colours = new color[delta];
    for (int i=0; i < delta; i++) {
      colours[i] = lerpColor(from, to, (float)i/(delta-1) );
    }
    return colours;
  }
  //colour gradient from String[]
  void colourByStrings(String[] topo_layer, color from, color to) {
    ...
    //initialise a list of colours to apply to each set item
    ...
    while (vitr.hasNext()) {
      ...
      int i = Util.ClosestPoint(new PVector(v.xf(), v.yf(), v.zf()), t.points, 100);
      //get landuse
